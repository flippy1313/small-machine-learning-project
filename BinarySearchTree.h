...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    else if(!less(node->datum, query) && !less(query, node->datum)){return node;}
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    && less(node->datum,val))return nullptr;
    else{
      if(less(val,node->datum)){
        if(node->left == nullptr || (less(node->left->datum, val)
        &&find_impl(node, val , less)->right == nullptr)){
          return node;
        }else if(!less(node->left->datum, val)&&!less(val, node->left->datum)){
          if(node->left->right == nullptr)return node;
          else return min_greater_than_impl(node->left->right, val, less);
        }
        else{
